        ...
        try {
            ...
            if ($request->hasFile('video')) {
                if ($section && $section->video && file_exists(public_path($section->video))) {
                    Helper::fileDelete(public_path($section->video));
                }
                $validatedData['video'] = Helper::fileUpload($request->file('video'), $this->section->value, time() . '_' . getFileName($request->file('video')));
            }
